...
class DraggableListState internal constructor(
	...
	internal fun onDragInterrupted() {
		...
		if (droppedIndex != null) {
			...
			scope.launch {
				// Publish the index only AFTER the offset holds the real drop position.
				// previousItemOffset is left at 0f by the previous settle, so announcing the
				// index first meant any frame landing before this coroutine body ran drew the
				// card at translationY 0 — already home. The spring then had nothing left to
				// animate and the drop read as an instant snap.
				previousItemOffset.snapTo(startOffset)
				previousIndexOfDraggedItem = droppedIndex
