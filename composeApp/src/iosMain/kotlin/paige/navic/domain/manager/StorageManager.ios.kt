...
import platform.Foundation.moveItemAtPath
...
actual class StorageManager {
	...
	actual fun getTempDownloadPath(songId: String, extension: String): String =
		getDownloadPath(songId, "$extension.part")

	actual fun finalizeFile(tempPath: String, path: String): Boolean {
		val manager = NSFileManager.defaultManager
		if (!manager.fileExistsAtPath(tempPath)) return false
		// moveItemAtPath won't overwrite, so clear the target first. Both paths sit in the same
		// directory, so the move is a rename: the real path only ever holds a COMPLETE file.
		if (manager.fileExistsAtPath(path)) {
			manager.removeItemAtPath(path, null)
		}
		val moved = manager.moveItemAtPath(tempPath, path, null)
		if (!moved) {
			// Never leave a partial behind pretending to be a download.
			manager.removeItemAtPath(tempPath, null)
		}
		return moved
	}
